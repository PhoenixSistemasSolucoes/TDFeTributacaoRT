...
//    Informado para abater as parcelas de antecipa��o de pagamento, conforme Art. 10. � 4�
...
//         infAdProd      := 'Informa��o Adicional do Produto';
      ...
      if rgReformaTributaria.ItemIndex = 0 then
      begin
        // Indicador de fornecimento de bem m�vel usado
        Prod.indBemMovelUsado := tieNenhum;

        // Valor total do Item, correspondente � sua participa��o no total da nota.
        // A soma dos itens dever� corresponder ao total da nota.
        vItem := 100;
        // Referenciamento de item de outro Documento Fiscal Eletr�nico - DF-e
      ...
      *)
      ...
      begin
        ...
        if rgReformaTributaria.ItemIndex = 0 then
        begin
          //  Informa��es do tributo: Imposto Seletivo s� para 2027 e para os
          ...
          // Preenchimento usando classe TDFeTributacaoRT - Padrão FLUENT
          TDFeTributacaoRT
            .ACBr(ACBrNFe1)
            .CST('000')
            .ClassTrib('000001')
            .IndicadorDoacao(tieSim)
            .AliquotaIBSUF(5.00)
            .AliquotaIBSMun(5.00)
            .AliquotaCBS(5.00)
            .Reducao(0)
          ...
          // Demais campos são preenchidos automaticamente pela classe TDFeTributacaoRT
        end;
      end;
    end;
    ...
    // Reforma Tributária - Totalização usando classe TDFeTributacaoRT
    if rgReformaTributaria.ItemIndex = 0 then
    begin
      TDFeTributacaoRT
        .ACBr(ACBrNFe1)
...
//Campos espec�ficos para agropecuario / defensivo
...
//Campos espec�ficos para agropecuario / guiaTransito
(*
...
//Campos espec�ficos para venda de combust�vel(distribuidoras)
  ...
  begin
    ...
    begin
      ...
      //Grupo de Tributa��o do ICMS Monof�sico sobre combust�veis.
      (*
      CST       := cst02;
      qBCMono   := 100;
      adRemICMS := 10;
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Aceite D�bito Apura��o Nota de Cr�dito', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Aceite D�bito Apura��o Nota de Cr�dito', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Aceite D�bito Apura��o Nota de Cr�dito', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Aceite D�bito Apura��o Nota de Cr�dito', 'UF do emitente do Evento', lUF)) then
       exit;

    lIndAceitacao := '1';
    if not(InputQuery('WebServices Eventos: Aceite D�bito Apura��o Nota de Cr�dito', 'Indicador de concord�ncia', lIndAceitacao)) then
       exit;
...
begin
  {
    ...
    S� ser� poss�vel realizar testes em ambiente de homologa��o a partir de 01/11/2021
    E enviar em produ��o a partir de 30/11/2021
  }
  ...
  TipoAutoriz := '0';
  if not(InputQuery(xTitulo, 'Tipo de Autoriza��o (0 ou 1)', TipoAutoriz)) then
     exit;
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Atualiza��o Data Prev. Entrega', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Atualiza��o Data Prev. Entrega', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Atualiza��o Data Prev. Entrega', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Atualiza��o Data Prev. Entrega', 'UF do emitente do Evento', lUF)) then
       exit;
...
begin
  lChave := '';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'Chave da NF-e', lChave)) then
     exit;
  lChave := Trim(OnlyNumber(lChave));
  lidLote := '1';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
     exit;
  lnSeqEvento := '1';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'N�mero sequencial do evento', lnSeqEvento)) then
     exit;
  lCNPJ := copy(lChave,7,14);
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'CNPJ ou o CPF do autor do Evento', lCNPJ)) then
     exit;
  lnProtEvento:='';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'Protocolo de autoriza��o do evento', lnProtEvento)) then
     exit;
  ltpEventoAut := '';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'Evento a ser cancelado', ltpEventoAut)) then
     exit;
  ltpAutor := '1';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'Autor do Evento a ser cancelado', ltpAutor)) then
     exit;
  lVerAplic := '1.00';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
     exit;
  lUF := 'SP';
  if not(InputQuery('WebServices Eventos: Cancelamento Gen�rico', 'UF do emitente do Evento', lUF)) then
     exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      infEvento.dhEvento := now;
      infEvento.cOrgao := 92; // C�digo do �rg�o de recep��o deste evento
...
begin
  ...
  begin
    ...
    MemoDados.Lines.Add('Reg. Apura��o: ' + ACBrNFe1.WebServices.ConsultaCadastro.RetConsCad.InfCad[i].xRegApur);
...
begin
  ...
  begin
    ...
    Sincrono := True;
    {
      A NT 2023/002 determina que o envio de NFC-e deve ocorrer de forma unit�ria,
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'UF do emitente do Evento', lUF)) then
       exit;
    ltpAutor := '1';
    if not(InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Autor do Evento', ltpAutor)) then
       exit;
    ...
    repeat
      lnItem := '1';
      if not InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Informe nItem', lnItem) then
        break;

      lvIBS := '1';
      if not InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Informe vIBS', lvIBS) then
        break;

      lvCBS := '1';
      if not InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Informe vCBS', lvCBS) then
        break;

      lqConsumo := '1';
      if not InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Informe qConsumo', lqConsumo) then
        break;

      luConsumo := 'UNIDAD';
      if not InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Informe uConsumo', luConsumo) then
        break;

      lchaveAcesso := '35251118760540000139550010000000011445690173';
      if not InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Informe chaveAcesso', lchaveAcesso) then
        break;
      ...
      InputQuery('WebServices Eventos: Destina��o Item consumo pessoal', 'Adicionar gConsumo? (S/N)', lAux);
...
begin
  ...
  begin
    ...
    if not(InputQuery('Concilia��o Financeira:', 'Numero do Protocolo', vProt)) then
      exit;
    ...
    with ACBrNFe1.EventoNFe.Evento.New do
    begin
      // Para o Evento de Cancelamento de Concilia��o Financeira
      // nSeqEvento sempre = 1
      infEvento.nSeqEvento := 1;
      infEvento.chNFe := Copy(ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.Id, 4, 44);
      infEvento.CNPJ := edtEmitCNPJ.Text;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        tPag := fpDinheiro;
        xPag := ''; // S� informar se a forma de pagamento for Outros.
        vPag := 100;
...

        {
         ...
         quando a emiss�o do documento fiscal ocorrer em estabelecimento distinto
        }
        CNPJPag := '';
        {
        ...
        UFPag := '';
        {
         CNPJ da institui��o financeira, de pagamento, adquirente ou subadquirente.
        }
        CNPJIF := '';
        {
         Bandeira da operadora de cart�o
        ...
        tBand := bcVisa;
        {
         N�mero de autoriza��o da opera��o com cart�es, PIX, boletos e
         outros pagamentos eletr�nicos
        }
        cAut := '';

        {
         Informar o CNPJ do estabelecimento benefici�rio do pagamento
        }
        CNPJReceb := '';
        {
         UF do CNPJ do estabelecimento benefici�rio do pagamento
        }
        UFReceb := '';
      end;
    end;
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'UF do emitente do Evento', lUF)) then
       exit;
    ...
    repeat
      lnItem := '1';
      InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Informe nItem', lnItem);

      lvIBS := '1';
      InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Informe vIBS', lvIBS);

      lvCBS := '1';
      InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Informe vCBS', lvCBS);

      lqNaoFornecida := '1';
      InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Informe qNaoFornecida', lqNaoFornecida);

      luNaoFornecida := 'UNIDAD';
      InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Informe uNaoFornecida', luNaoFornecida);
      ...
      InputQuery('WebServices Eventos: Fornecimento n�o realizado, pag. antecip.', 'Adicionar gItemNaoFornecido? (S/N)', lAux);
    until (UpperCase(lAux) <> 'S');
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Imobiliza��o de Item', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Imobiliza��o de Item', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Imobiliza��o de Item', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Imobiliza��o de Item', 'UF do emitente do Evento', lUF)) then
       exit;
    ...
    repeat
      lnItem := '1';
      InputQuery('WebServices Eventos: Imobiliz���o de Item', 'Informe nItem', lnItem);

      lvIBS := '1';
      InputQuery('WebServices Eventos: Imobiliz���o de Item', 'Informe vIBS', lvIBS);

      lvCBS := '1';
      InputQuery('WebServices Eventos: Imobiliz���o de Item', 'Informe vCBS', lvCBS);

      lqImobilizado := '1';
      InputQuery('WebServices Eventos: Imobiliz���o de Item', 'Informe qtde', lqImobilizado);

      luImobilizado := 'UNIDAD';
      InputQuery('WebServices Eventos: Imobiliz���o de Item', 'Informe unidade', luImobilizado);
      ...
      InputQuery('WebServices Eventos: Imobiliz���o de Item', 'Adicionar gImobilizacao? (S/N)', lAux);
    until (UpperCase(lAux) <> 'S');
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Importar��o ALC/ZFM', 'UF do emitente do Evento', lUF)) then
       exit;
    ...
    repeat
      lnItem := '1';
      InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Informe nItem', lnItem);

      lvIBS := '1';
      InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Informe vIBS', lvIBS);

      lvCBS := '1';
      InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Informe vCBS', lvCBS);

      lqtde := '1';
      InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Informe qtde', lqtde);

      lunidade := 'UNIDAD';
      InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Informe unidade', lunidade);
      ...
      InputQuery('WebServices Eventos: Importa��o ALC/ZFM', 'Adicionar gConsumo? (S/N)', lAux);
    until (UpperCase(lAux) <> 'S');
...
begin
  ...
  Titulo := 'Manifesta��o Destinatario - ' + StringReplace(TpEventoToDescStr(AtpEvento),'ManifDest','',[rfReplaceAll]);
  ...
  begin
    {
     Como os Eventos de Manifesta��o do Destinat�rio s�o enviados para o
    ...
    infEvento.dhEvento := now;
    {
     Tipos de Eventos de Manifesta��o do Destinat�rio:
       teManifDestConfirmacao      = Confirma��o da Opera��o
       teManifDestCiencia          = Ci�ncia da Opera��o
       teManifDestDesconhecimento  = Desconhecimento da Opera��o
       teManifDestOperNaoRealizada = Opera��o N�o Realizada
    }
    infEvento.tpEvento := AtpEvento;

    {
     Se o tipo for: teManifDestOperNaoRealizada, � obrigat�rio informar a justificativa
     Se o tipo for: teManifDestDesconhecimento, � opcional informar a justificativa
    }
    InfEvento.detEvento.xJust := Justificativa;
  end;
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Manifesta��o Transf. Cr�d. IBS e CBS', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Manifesta��o Transf. Cr�d. IBS e CBS', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Manifesta��o Transf. Cr�d. IBS e CBS', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Manifesta��o Transf. Cr�d. IBS e CBS', 'UF do emitente do Evento', lUF)) then
       exit;

    lIBSCBS := '';
    if not(InputQuery('WebServices Eventos: Manifesta��o Transf. Cr�d. IBS e CBS', 'Defina entre "IBS" ou "CBS"', lIBSCBS)) then
       exit;

    lIndAceitacao := '0';
    if not(InputQuery('WebServices Eventos: Manifesta��o Transf. Cr�d. IBS e CBS', 'Indicador de aceita��o do valor', lindAceitacao)) then
       exit;
    ...
    if UpperCase(lIBSCBS) = 'IBS' then
      ...
      raise Exception.Create('� preciso definir o tpEvento escolhendo entre IBS ou CBS');
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Informa��o Efetiv. Pag. Integral Libera Cred. Pres.', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Informa��o Efetiv. Pag. Integral Libera Cred. Pres.', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Informa��o Efetiv. Pag. Integral Libera Cred. Pres.', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Informa��o Efetiv. Pag. Integral Libera Cred. Pres.', 'UF do emitente do Evento', lUF)) then
       exit;
...
begin
  ...
  begin
    ...
    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Perecimento em transp. pelo contratado fornecedor', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'UF do emitente do Evento', lUF)) then
       exit;
    ...
    repeat
      lnItem := '1';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe nItem', lnItem);

      lvBC := '1';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe vBC', lvBC);

      lIBScCredPres := '01';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe cCredPres do IBS', lIBScCredPres);

      lIBSpCredPres := '1';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe pCredPres do IBS', lIBSpCredPres);

      lIBSvCredPres := '1';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe vCredPres do IBS', lIBSvCredPres);

      lCBScCredPres := '01';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe cCredPres da CBS', lCBScCredPres);

      lCBSpCredPres := '1';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe pCredPres da CBS', lCBSpCredPres);

      lCBSvCredPres := '1';
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Informe vCredPres da CBS', lCBSvCredPres);
      ...
      InputQuery('WebServices Eventos: Solic. Apropria��o Cr�d. Presumido', 'Adicionar gCredPres? (S/N)', lAux);
    until (UpperCase(lAux) <> 'S');
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'UF do emitente do Evento', lUF)) then
       exit;
    ...
    repeat
      lnItem := '1';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'Informe nItem', lnItem);

      lvCredIBS := '1';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'Informe vCredIBS', lvCredIBS);

      lvCredCBS := '1';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'Informe vCredCBS', lvCredCBS);
      ...
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Bens Adiq.', 'Adicionar gCredito? (S/N)', lAux);
    until (UpperCase(lAux) <> 'S');
...
begin
  ...
  begin
    ...
    lidLote := '1';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Identificador de controle do Lote de envio do Evento', lidLote)) then
       exit;

    lnSeqEvento := '1';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Numero sequencial do evento', lnSeqEvento)) then
       exit;

    lVerAplic := '1.00';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Vers�o do Aplicativo do emitente', lVerAplic)) then
       exit;

    lUF := 'SP';
    if not(InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'UF do emitente do Evento', lUF)) then
       exit;
    ...
    repeat
      lnItem := '1';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Informe nItem', lnItem);

      lvIBS := '1';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Informe vIBS', lvIBS);

      lvCBS := '1';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Informe vCBS', lvCBS);

      lqComb := '1';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Informe qtde', lqComb);

      luComb := 'UNIDAD';
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Informe unidade', luComb);
      ...
      InputQuery('WebServices Eventos: Solicita��o Aprop. Cr�d. Combust�vel', 'Adicionar gConsumoComb? (S/N)', lAux);
    until (UpperCase(lAux) <> 'S');
...
begin
  ...
  // IdCSRT e CSRT do Respons�vel T�cnico, no momento s� a SEFAZ-PR esta exigindo
  ...
  begin
    ...
    {
      A Configura��o abaixo utilizanda em conjunto com o TipoDANFE = tiSimplificado
      para impress�o do DANFE Simplificado - Etiqueta (Fortes Report)
